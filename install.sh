...
runin() {
	...
	{ "$@" 2>>../s1zex-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../s1zex-install.log
	done
}
...
printf "\n\e[1;35;47m                   \e[0m"
printf "\n\e[1;35;47m s1zex s1zex s1zex \e[0m"
printf "\n\e[1;35;47m s1zex s1zex s1zex \e[0m"
printf "\n\e[1;35;47m                   \e[0m"
...
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" s1zex-install.log
fi

if [ -d "userbot/s1zex" ]; then
	cd userbot || {
...
if [ -f ".setup_complete" ]; then
	# If s1zex is already installed by this script
	PYVER=""
...
echo "Installing..." >s1zex-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/s1zexxx/userbot/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf userbot
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/s1zexxx/userbot || {
