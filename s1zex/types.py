...
s1zexReplyMarkup = typing.Union[typing.List[typing.List[dict]], typing.List[dict], dict]
...
class Module:
    ...
    @property
    def s1zex_commands(self) -> typing.Dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def s1zex_inline_handlers(self) -> typing.Dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def s1zex_callback_handlers(self) -> typing.Dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def s1zex_watchers(self) -> typing.Dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @s1zex_commands.setter
    def s1zex_commands(self, _):
        pass
    ...
    @s1zex_inline_handlers.setter
    def s1zex_inline_handlers(self, _):
        pass
    ...
    @s1zex_callback_handlers.setter
    def s1zex_callback_handlers(self, _):
        pass
    ...
    @s1zex_watchers.setter
    def s1zex_watchers(self, _):
        pass
    ...
    async def animate(
        ...
        *,
        ...
        with contextlib.suppress(AttributeError):
            _s1zex_client_id_logging_tag = copy.copy(self.client.tg_id)
    ...
    async def _decline(
        ...
        self._db.set(
            "s1zex.main",
            "declined_joins",
            list(set(self._db.get("s1zex.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def request_join(
        ...
        if channel.id in self._db.get("s1zex.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        self.s1zex_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "s1zex_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?s1zex_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?s1zex_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires s1zex version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"s1zex.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("s1zex.main", "stats", True)
            and url is not None
...
class DragonModule:
    ...
    def __init__(self):
        ...
        self.s1zex_inline_handlers = {}
        self.callback_handlers = {}
        self.s1zex_callback_handlers = {}
