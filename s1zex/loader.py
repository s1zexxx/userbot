...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _s1zex_client_id_logging_tag = copy.copy(
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?s1zex$", from_id=5560308564)
...
class Modules:
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.s1zex_commands)
                inline_handlers.update(module.s1zex_inline_handlers)
                callback_handlers.update(module.s1zex_callback_handlers)
                watchers.extend(module.s1zex_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.s1zex_commands))
            )

        for _command, cmd in instance.s1zex_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.s1zex_commands:
                self.add_alias(alias, cmd)

        self.register_inline_stuff(instance)

    def register_inline_stuff(self, instance: Module):
        for name, func in instance.s1zex_inline_handlers.copy().items():
        ...
        for name, func in instance.s1zex_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.s1zex_watchers.values():
    ...
    def get_prefix(self, userbot: typing.Optional[str] = None) -> str:
        """Get prefix for specific userbot. Pass `None` to get s1zex prefix"""
        if userbot == "dragon":
            key = "dragon.prefix"
            default = ","
        else:
