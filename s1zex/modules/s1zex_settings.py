...
@loader.tds
class s1zexSettingsMod(loader.Module):
    """Advanced settings for s1zex Userbot"""
    ...
    strings = {
        ...
        "inline_settings": "⚙️ <b>Here you can configure your s1zex settings</b>",
        "confirm_update": (
        ...
        "nonick_warning": (
            ...
            "You may get muted in s1zex chats. Change prefix or "
            "disable NoNick!"
        ),
        "reply_required": (
        ...
        "deauth_confirm": (
            "⚠️ <b>This action will fully remove s1zex from this account and can't be"
            " reverted!</b>\n\n<i>- s1zex chats will be removed\n- Session will be"
            " terminated and removed\n- s1zex inline bot will be removed</i>"
        ),
        "deauth_confirm_step2": (
            "⚠️ <b>Are you really sure you want to delete s1zex?</b>"
        ),
        "deauth_yes": "I'm sure",
        ...
        "uninstall": "😢 <b>Uninstalling s1zex...</b>",
        "uninstalled": (
            "😢 <b>s1zex uninstalled. Web interface is still active, you can add another"
            " account</b>"
        ),
        "cmd_nn_list": (
        ...
        "privacy_leak": (
            "⚠️ <b>This command gives access to your s1zex web interface. It's not"
        ...
        "opening_tunnel": "🔁 <b>Opening tunnel to s1zex web interface...</b>",
        "tunnel_opened": "🎉 <b>Tunnel opened. This link is valid for about 1 hour</b>",
        "web_btn": "🌍 Web interface",
        "btn_yes": "🚸 Open anyway",
        "btn_no": "🔻 Cancel",
        "lavhost_web": (
            "✌️ <b>This link leads to your s1zex web interface on lavHost</b>\n\n<i>💡"
    ...
    strings_ru = {
        ...
        "inline_settings": "⚙️ <b>Здесь можно управлять настройками s1zex</b>",
        ...
        "_cls_doc": "Дополнительные настройки s1zex",
        "nonick_warning": (
            "Внимание! Ты включил NoNick со стандартным префиксом! "
            "Тебя могут замьютить в чатах s1zex. Измени префикс или "
        ...
        "deauth_confirm": (
            "⚠️ <b>Это действие полностью удалит s1zex с этого аккаунта! Его нельзя"
            " отменить</b>\n\n<i>- Все чаты, связанные с s1zex будут удалены\n- Сессия"
            " s1zex будет сброшена\n- Инлайн бот s1zex будет удален</i>"
        ),
        "deauth_confirm_step2": "⚠️ <b>Ты точно уверен, что хочешь удалить s1zex?</b>",
        "deauth_yes": "Я уверен",
        ...
        "uninstall": "😢 <b>Удаляю s1zex...</b>",
        "uninstalled": (
            "😢 <b>s1zex удалён. Веб-интерфейс все еще активен, можно добавить другие"
            " аккаунты!</b>"
        ),
        "cmd_nn_list": (
        ...
        "privacy_leak": (
            "⚠️ <b>Эта команда дает доступ к веб-интерфейсу s1zex. Ее выполнение в"
        ...
        "opening_tunnel": "🔁 <b>Открываю тоннель к веб-интерфейсу s1zex...</b>",
        "tunnel_opened": (
        ...
        "btn_no": "🔻 Закрыть",
        "lavhost_web": (
            "✌️ <b>По этой ссылке ты попадешь в веб-интерфейс s1zex на"
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"s1zex-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "s1zex" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.owner
    @loader.command(ru_doc="Удалить s1zex")
    async def uninstall_s1zex(self, message: Message):
        """Uninstall s1zex"""
    ...
    @loader.owner
    @loader.command(ru_doc="Открыть тоннель к веб-интерфейсу s1zex")
    async def weburl(self, message: Message, force: bool = False):
        """Opens web tunnel to your s1zex web interface"""
        if "LAVHOST" in os.environ:
            form = await self.inline.form(
                ...
                reply_markup={
                    ...
                    "url": await main.s1zex.web.get_url(proxy_pass=False),
                },
        ...
        url = await main.s1zex.web.get_url(proxy_pass=True)
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "clear_entity_cache":
                result = (
                    f"Dropped {len(self._client._s1zex_entity_cache)} cache records"
                )
                self._client._s1zex_entity_cache = {}
            elif method == "clear_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._s1zex_fulluser_cache)} cache records"
                )
                self._client._s1zex_fulluser_cache = {}
            elif method == "clear_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._s1zex_fullchannel_cache)} cache"
                    " records"
                )
                self._client._s1zex_fullchannel_cache = {}
            elif method == "clear_perms_cache":
                result = f"Dropped {len(self._client._s1zex_perms_cache)} cache records"
                self._client._s1zex_perms_cache = {}
            elif method == "clear_cache":
                result = (
                    f"Dropped {len(self._client._s1zex_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._s1zex_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._s1zex_fullchannel_cache)} fullchannel cache"
                    " records"
                )
                self._client._s1zex_entity_cache = {}
                self._client._s1zex_fulluser_cache = {}
                self._client._s1zex_fullchannel_cache = {}
                self._client.s1zex_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._s1zex_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._s1zex_fulluser_cache)} records\nFullchannel"
                    f" cache: {len(self._client._s1zex_fullchannel_cache)} records"
                )
