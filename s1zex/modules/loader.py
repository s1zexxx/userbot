...
class LoaderMod(loader.Module):
    ...
    strings = {
        ...
        "inline_init_failed": (
            ...
            " s1zex inline feature and initialization of InlineManager"
    ...
    strings_ru = {
        ...
        "inline_init_failed": (
            ...
            " s1zexInline, а инициализация менеджера инлайна неудачна</b>\n<i>Попробуй"
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                ...
                [
                    "https://github.com/s1zexxx/Modules/raw/main"
    ...
    async def client_ready(self):
        self._token_msg = (await self._client.get_messages("@s1zexbee", ids=[10]))[0]

        self.allmodules.add_aliases(self.lookup("settings").get("aliases", {}))

        main.s1zex.ready.set()
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?s1zex_min", doc):
            ver = re.search(r"# ?scope: ?s1zex_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        if is_dragon:
            ...
            module_name = f"s1zex.modules.{uid}"
            doc = geek.compat(doc)
        ...
        async with (dragon.import_lock if is_dragon else lambda _: FakeLock())(
                            ...
                            requirements = [
                                {
                                    ...
                                    "pil": "Pillow",
                                    "telethon": "s1zex-TL",
                                    "s1zextl": "s1zex-TL-New",
                                    "pyrogram": "s1zex-Pyro",
                                    "s1zexpyro": "s1zex-Pyro-New",
                ...
                try:
                    try:
                        ...
                        async def inner_proxy():
                            ...
                            while True:
                                if hasattr(instance, "s1zex_wait_channel_approve"):
                                    if message:
                                        (
                                            module,
                                            channel,
                                            reason,
                                        ) = instance.s1zex_wait_channel_approve
                                        message = await utils.answer(
                                            message,
            ...
            try:
                developer_entity = await (
                    ...
                    if (
                        developer in self._client._s1zex_entity_cache
                        and getattr(
