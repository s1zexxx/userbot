...
class UpdateNotifierMod(loader.Module):
    """Tracks latest s1zex releases, and notifies you, if update is required"""

    strings = {
        "name": "UpdateNotifier",
        "update_required": (
            "🥀 <b>s1zex Update available!</b>\n\nNew GitHub commit pushed.\n💿"
            " <b>s1zex <s>{}</s> -> {}</b>\n\nChanges: 👇🏻\n\n{}"
        ),
        "more": "\n<i><b>🎥 And {} more...</b></i>",
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "s1zex_update"},
                {"text": self.strings("ignore"), "data": "s1zex_upd_ignore"},
            ]
        )
        ...
        if self._pending not in [utils.get_git_hash(), self._notified]:
            m = await self.inline.bot.send_message(
                ...
                self.strings("update_required").format(
                    ...
                    '<a href="https://github.com/s1zexxx/userbot/compare/{}...{}">{}</a>'.format(
    ...
    async def update(self, call: InlineCall):
        ...
        if call.data not in {"s1zex_update", "s1zex_upd_ignore"}:
            return

        if call.data == "s1zex_upd_ignore":
