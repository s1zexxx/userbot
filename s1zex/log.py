...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of s1zex and Dragon modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (
                module.startswith("s1zex.modules")
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.s1zex_caller or client_id == record.s1zex_caller)
        ]
    ...
    def _gen_web_debug_button(self, item: s1zexException) -> list:
        if not item.sysinfo:
            return []
    ...
    async def _start_debugger(self, call: "InlineCall", item: s1zexException):  # type: ignore
        ...
        async with self._send_lock:
            ...
            self._exc_queue = {
                client_id: [
                    ...
                    for item in self.tg_buff
                    if isinstance(item[0], s1zexException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_s1zex_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_s1zex_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        record.s1zex_caller = caller

        if record.levelno >= self.tg_level:
            if record.exc_info:
                self.tg_buff += [
                    (
                        s1zexException.from_exc_info(
                            *record.exc_info,
