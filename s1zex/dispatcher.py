...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.s1zex_me.id
        self._cached_usernames = [
            (
                self._client.s1zex_me.username.lower()
                if self._client.s1zex_me.username
                else str(self._client.s1zex_me.id)
            )
        ]

        self._cached_usernames.extend(
            getattr(self._client.s1zex_me, "usernames", None) or []
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("s1zex-")
            and message.chat.title != "s1zex-logs"
        ):
            if not watcher:
    ...
    async def future_dispatcher(
        ...
        *args,
        ...
        _s1zex_client_id_logging_tag = copy.copy(self.client.tg_id)  # skipcq
        try:
            await func(message)
        except Exception as e:
