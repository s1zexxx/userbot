...
from .types import s1zexReplyMarkup, ListLike, Module
...
async def answer(
    ...
    response: str,
    *,
    reply_markup: typing.Optional[s1zexReplyMarkup] = None,
    **kwargs,
) -> typing.Union[InlineCall, InlineMessage, Message]:
    """
    Use this to give the response to a command
    :param message: Message to answer to. Can be a tl message or s1zex inline object
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "s1zex_grepped"):
            try:
...
async def convert_folders(client):
    ...
    try:
        folder = next(
            ...
            if hasattr(folder, "title") and folder.title == "s1zex"
        )
    except Exception:
        folder = None
...
async def asset_channel(
    ...
    *,
    ...
    # legacy s1zex / s1zex chats conversion to s1zex
    if title.startswith("s1zex-"):
        title = title.replace("s1zex-", "s1zex-")

    async for d in client.iter_dialogs():
        if (d.title == title) or (
            (d.title.replace("s1zex-", "s1zex-") == title)
            if d.title.startswith("s1zex-")
            else False
        ):
            if d.title.startswith("s1zex-"):
    ...
    if _folder:
        ...
        _folder = "s1zex" if _folder == "s1zex" else _folder
...
def get_platform_emoji(client: typing.Optional[CustomTelegramClient] = None) -> str:
    ...
    """
    BASE = ("<emoji document_id=5281005969420066218>🥀</emoji> <b>s1zex</b>",)

    if client and (
        client.loader.db.get("s1zex.translations", "lang", False) or ""
    ).startswith("ar"):
...
def get_commit_url() -> str:
    """
    Get current s1zex git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return f'<a href="https://github.com/s1zexxx/userbot/commit/{hash_}">#{hash_[:7]}</a>'
    except Exception:
        return "Unknown"
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/s1zexxx/userbot/commit/{hash_}" if hash_ else "",
    )
