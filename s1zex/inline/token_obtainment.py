...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("s1zex.inline", "custom_bot", False):
                username = self._db.get("s1zex.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            for msg in [
                f"🥀 s1zex Userbot of {self._name}"[:64],
                username,
                "/setuserpic",
                username,
            ]:
    ...
    async def _assert_token(
        ...
        logger.info("s1zex can't find token in database, taking it from BotFather")
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@s1zex_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    self._db.set("s1zex.inline", "bot_token", token)
                    self._token = token
