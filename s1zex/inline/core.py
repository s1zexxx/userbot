...
class InlineManager(
    ...
    def __init__(
        ...
        self._token = db.get("s1zex.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.s1zex_me)
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start s1zex init")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start s1zex init"
                )
            except Exception:
