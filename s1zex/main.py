...
class s1zex:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("s1zex-") and f.endswith(".session"),
    ...
    async def save_client_session(self, client: CustomTelegramClient):
        ...
        client.s1zex_db = database.Database(client)
        await client.s1zex_db.init()

    async def _web_banner(self):
        """Shows web banner"""
        logging.info("✅ s1zex is now can be setted up in browser!")
        ...
        if not self.web:
            try:
                ...
                client = CustomTelegramClient(
                    ...
                    device_model=(f"s1zex"),
                    app_version=(f"s1zex v{netver[0]}.{netver[1]}.{netver[2]}"),
                )
    ...
    async def _badge(self, client: CustomTelegramClient):
     ...
     _______.__   ________   __________   ___
    /       /_ | |       /  |   ____\  \ /  /
   |   (----`| | `---/  /   |  |__   \  V  /
    \   \    | |    /  /    |   __|   >   <
.----)   |   | |   /  /----.|  |____ /  .  \
|_______/    |_|  /________||_______/__/ \__\
